...
class Hill:
    ...
    def inverseKey(key):
        k = Hill.getKeyMatrix(key)
        d = ((k[0][0] * k[1][1]) - (k[0][1] * k[1][0]))
        d = Hill.mod_inverse(d, mod)
    ...
    def encrypt(messageMatrix, keyMatrix):
    ...
    def HillCipher(message, keyMatrix, alpha):
        ...
        cipherMatrix = Hill.encrypt(messageVector, keyMatrix)
        ...
        for i in range(mod):
            ...
            for l in range(mod):
                for m in range(mod):
                    for n in range(mod):
                        keyMatrix = [[i, l], [m, n]]
                        d = Hill.HillCipher(message, keyMatrix, ALPHABET)
                        if (crib in d):
                            print(d, keyMatrix)
...
key = "12 2 1 5"
keyMatrix = Hill.getKeyMatrix(key)
# keyMatrix = Hill.inverseKey(key)
