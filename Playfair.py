...
class Playfair:
    def decode(string, word):
        grid = Playfair.makeGrid(word)
        # for item in grid:
        #  print(item)
        return Playfair.decodeWithGrid(string, grid)
    ...
    def decodeWithGrid(string, grid):
        ...
        for item in cipher:
            first = Playfair.getLocation(list(item)[0], grid)
            second = Playfair.getLocation(list(item)[1], grid)
...
# Playfair.decode(cipher, keyword)
# Playfair.decodeWithGrid(cipher)
# Playfair.crackWithCrib(c, crib)
# Playfair.cribWithMoreWords(c,crib)
